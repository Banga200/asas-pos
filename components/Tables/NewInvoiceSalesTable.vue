...
const emit = defineEmits(["increseField",'setQuantity', 'setLineNo', "setUnit", 'setTotal', 'setVatValue','removeItem', 'setTax', 'setDiscount'])
const names = ref([
    {
        name: '',
        unitName: ''
...
function setTotal(qty,price, index) {
    const vat = Items.value[index] // اجيب الصنف من Array حق الاصناف عشان اجيب VatValue
    ...
    <table class="newInvoiceSales">
        ...
        <tbody>
            <tr v-for="(row,i) in items" :key="i">
                ...
                {{ getLineNo(i) }}
                <td>{{ i+1 }}</td>
                <td class="item-name"><ComboBoxCell :placeholder="'اسم الصنف'" v-model:input="names[i].name" :items="Items"  :index="i" v-model:selectedItemID="row.ItemID" @increseField="increseItem" @setVatValue="getVatValue"/></td>
                <td class="unit"><ComboBoxCell :placeholder="'الوحدة'" v-model:input="names[i].unitName" v-model:selectedID="row.UnitID" :index="i" :itemID="row.ItemID" @selectedUnit="getUnit"/></td>
                <td class="textField"><TextInputCell v-model:input="row.Quantity" @getValue="getQuantityValue" :index="i"/></td>
                <td class="textField"><TextInputCell v-model:input="row.Price" /></td>
                <td class="textField"><TextInputCell v-model:input="row.Total" :readOnly="true"/></td>
                <td class="textField"><TextInputCell v-model:input="row.Discount" @getValue="getDiscount" :index="i"/></td>
                <td class="textField"><TextInputCell v-model:input="row.Tax" :readOnly="true" @getValue="getTax" :index="i"/></td>
                <td class="textField"><TextInputCell v-model:input="row.Net" :readOnly="true"/></td>
